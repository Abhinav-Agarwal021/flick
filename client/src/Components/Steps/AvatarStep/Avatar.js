...
export const Avatar = (props) => {
    ...
    const { fullName, phone, avatar } = useSelector((state) => state.details)
    ...
    const submit = async () => {
        ...
        setLoading(true)
        try {
            const { data } = await activateUser({ fullName, phone, avatar });
            if (data.auth) {
                dispatch(setAuth(data))
            }
            await updateRoom({ dm: true, members: user._id });
        } catch (error) {
            console.log(error)
        } finally {
            setLoading(false)
